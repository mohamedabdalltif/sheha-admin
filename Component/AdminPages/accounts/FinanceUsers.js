...
const FinanceUsers = ({navigation, route}) => {
  ...
  function getFinanceUsers() {
    let domain = Accountspart.domain;

    Axios.get(domain + 'get_finance_users.php')
      .then((res) => {
        if (Array.isArray(res.data)) {
          setFinanceUser(res.data);
        }
      })
      .finally(() => {
        ...
        renderItem={({item, index}) => (
          <Animatable.View animation={'fadeInRightBig'} delay={index * 100}>
            <Button
              ...
              onPress={() => {
                navigation.navigate('FinanceUsersMonth', {
                  passData: item,
                });
                // onPress(item);
              }}
